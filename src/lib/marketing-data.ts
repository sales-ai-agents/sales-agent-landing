...
// Russian FAQ entries — only used in JSON-LD schema for SEO
export const FAQ_ENTRIES_RU: readonly FaqEntry[] = [
  {
    question: "Что такое Calls4U и как работает ИИ-агент для звонков?",
    answer:
      "Calls4U — это голосовой ИИ-агент, который автоматизирует рутинные звонки клиентам: подтверждает записи, напоминает о визитах, уточняет заказы и передаёт результат в CRM. Запуск за 1 день без программистов.",
    textAnswer:
      "Calls4U — это голосовой ИИ-агент, который автоматизирует рутинные звонки клиентам: подтверждает записи, напоминает о визитах, уточняет заказы и передаёт результат в CRM. Запуск за 1 день без программистов.",
  },
  {
    question: "Для какого бизнеса подходит голосовой ИИ-агент?",
    answer:
      "Для любого бизнеса с рутинными звонками: салоны красоты, клиники, СТО, интернет-магазины, службы доставки, логистика. Агент подтверждает записи, уточняет заказы и квалифицирует заявки.",
    textAnswer:
      "Для любого бизнеса с рутинными звонками: салоны красоты, клиники, СТО, интернет-магазины, службы доставки, логистика. Агент подтверждает записи, уточняет заказы и квалифицирует заявки.",
  },
  {
    question: "Сколько стоит ИИ-агент для звонков?",
    answer:
      "Оплата только за использованные минуты разговора. Все сценарии, интеграции с CRM и тестовый баланс уже включены в тарифы. Первый тест — бесплатно.",
    textAnswer:
      "Оплата только за использованные минуты разговора. Все сценарии, интеграции с CRM и тестовый баланс уже включены в тарифы. Первый тест — бесплатно.",
  },
] as const;
